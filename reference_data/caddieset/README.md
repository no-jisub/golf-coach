...
- 바깥 관찰 범위 밖: `warning / outside_observed`
- 관절 또는 어드레스 기준 부족: `unavailable`

화면에는 가장 크게 벗어난 항목부터 최대 3개의 교정 문구가 표시됩니다. 현재 웹캠 프레임은 좌우 반전되므로 위치 교정 문구는 사용자가 보는 화면의 왼쪽·오른쪽 방향으로 다시 변환합니다.

CaddieSet은 지표 설명을 공개하지만 원본 계산 코드는 공개하지 않습니다. 현재 지표 계산은 논문의 관절 정의에 맞춘 2D 근사식이므로 실제 골프 코치 검수 전에는 관찰 기반 보조 판정으로 사용해야 합니다.
