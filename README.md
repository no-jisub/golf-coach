...
후보 위치에서 시작해 영상 전체를 프레임 단위로 탐색하고 8단계 정답을 선택:
...
- `1`~`8`: 단계 바로 이동
- `c`: 자동 후보 프레임 순환
- `m`: 현재 프레임을 현재 단계 정답으로 지정
- `+`/`-`: 확대/축소
- `q`: 진행 상태 저장 후 종료

미완료 검수는 `analysis_sessions/stage_review_progress`에 저장되어 다음 실행에서
같은 단계와 프레임부터 재개됩니다. 8단계를 모두 선택한 경우에만 시간순·중복·범위를
검증한 뒤 정답 매니페스트를 갱신합니다.
